...
function Account() {
  ...
  const handleMathsAttendance = async()=>{
    if (attendanceTaken) {
      alert('Your attendance for today has been marked, till tomorrow');
      return;
    }
    try {
      ...
      const attendanceRef = collection(db, 'maths');
      const q = query(attendanceRef, where('email', '==', email), limit(1));
      const querySnapshot = await getDocs(q);

      if (querySnapshot.empty) {
        await addDoc(collection(db, 'maths'), {
          email,
          latitude,
          longitude,
        });
  ...
  const handleEnglishAttendance = async()=>{
    if (attendanceTaken) {
      alert('Your attendance for today has been marked, till tomorrow');
      return;
    }
    try {
      ...
      const attendanceRef = collection(db, 'english');
      const q = query(attendanceRef, where('email', '==', email), limit(1));
      const querySnapshot = await getDocs(q);
...
<div className='text-left'>Mathematics</div>
<div className='text-right'>
  <Link className='bg-button text-btnText px-5 py-2 ml-2 rounded-2xl shadow-lg hover:shadow-2xl mt-2' onClick={handleMathsAttendance}>Click Here</Link>
</div>
</div>
...
<div className='text-right'>
<Link className='bg-button text-btnText px-5 py-2 ml-2 rounded-2xl shadow-lg hover:shadow-2xl mt-2' onClick={handleEnglishAttendance}>Click Here</Link>
</div>
</div>
